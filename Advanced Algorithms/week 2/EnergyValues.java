...
class EnergyValues {
    ...
    static Position SelectPivotElement(double a[][], boolean used_raws[], boolean used_columns[]) {
        ...
        while (used_raws[pivot_element.raw])
            ++pivot_element.raw;
        while (used_columns[pivot_element.column] && a[pivot_element.raw][pivot_element.column] == 0)
            //the second term prevents a pivot value of 0
            ++pivot_element.column;           
        
        //System.out.println("Pivot ["+pivot_element.raw+", "+pivot_element.column+"]");
    ...
    static void ProcessPivotElement(double a[][], double b[], Position pivot_element) {
        ...
        //scale pivot to 1
        double tmp = a[pivot_element.raw][pivot_element.column];
        if (tmp != 0){//this is a guard in case the pivot value is 0. this should be addressed in pivot selection
        
            for (int i = 0; i < size; i++){
                a[pivot_element.raw][i] = a[pivot_element.raw][i] / tmp;
            }        
            b[pivot_element.raw] = b[pivot_element.raw] / tmp;

            //System.out.println("Scale Pivot Row");
            //PrintMatrixes(a,b);
        
            //subtract rows 
            for (int i = 0; i < size; i++){
                //subtract a scaled value of row 0 from the others to make a[i][pivot_element.column] = 0
                if (i == pivot_element.raw){continue;}
                double rowValue = a[i][pivot_element.column]; 
                for (int j = 0; j < size; j++){
                    a[i][j] = a[i][j] - a[pivot_element.raw][j]*rowValue;
                }
                b[i] = b[i] - b[pivot_element.raw]*rowValue;
            }        
        }
        //System.out.println("Row Subtraction");
        //PrintMatrixes(a,b);        
    }
    ...
    static double[] SolveEquation(Equation equation) {
        ...
        boolean[] used_columns = new boolean[size]; //initialized to false
        boolean[] used_raws = new boolean[size]; //initialized to false
    ...
    static void PrintMatrixes(double a[][], double b[]) {
        int size = a.length;
        for (int i = 0; i < size; i++){
            for (int j = 0; j < size; j++){
