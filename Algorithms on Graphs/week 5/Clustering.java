...
public class Clustering {
    ...
    private static double clustering(int[] x, int[] y, int k) {
        ...
        ArrayList<Edge> MSTedges = new ArrayList<Edge>(); //use ArrayList to store the MST Edges
        /*
        ...
        */
        ...
        for (int i=0; i<MSTedges.size(); i++){
            Edge currentEdge = MSTedges.get(i);
            System.out.println(currentEdge.u+" "+currentEdge.v+" "+currentEdge.distance);
        }
        */
        //The maximim dustance d for a cluster of size k can be determined from MSTedge. Answer is MSTedges.get(MSTedges.size()-k+1).distance
        Edge currentEdge = MSTedges.get(MSTedges.size()-k+1);
        result = currentEdge.distance;        
        return result;                       
    }
